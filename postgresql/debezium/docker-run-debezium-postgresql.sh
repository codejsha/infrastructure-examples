...
function docker_run_debezium_postgresql13 {
    ...
    # docker container run -i --rm debezium/postgres:13 cat /usr/share/postgresql/postgresql.conf.sample > debezium-postgresql.conf
...
function docker_run_debezium_postgresql12 {
    local POSTGRESQL_VOLUME_DIR="/mnt/volume/postgresql"
    sudo mkdir -p ${POSTGRESQL_VOLUME_DIR}/data
    # docker container run -i --rm debezium/postgres:12 cat /usr/share/postgresql/postgresql.conf.sample > debezium-postgresql.conf
    sudo /bin/cp -f debezium-postgresql.conf ${POSTGRESQL_VOLUME_DIR}/postgresql.conf
...
function docker_run_debezium_postgresql11 {
    local POSTGRESQL_VOLUME_DIR="/mnt/volume/postgresql"
    sudo mkdir -p ${POSTGRESQL_VOLUME_DIR}/data
    # docker container run -i --rm debezium/postgres:11 cat /usr/share/postgresql/postgresql.conf.sample > debezium-postgresql.conf
    sudo /bin/cp -f debezium-postgresql.conf ${POSTGRESQL_VOLUME_DIR}/postgresql.conf
