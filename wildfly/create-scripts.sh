...
JAVA_OPTS="\${JAVA_OPTS} -Xlog:gc*=info:file=\${LOG_DIR}/gc.\${INSTANCE_NAME}.log:time,pid,tid,level,tags"
# JAVA_OPTS="\${JAVA_OPTS} -Xlog:gc*=info:file=\${LOG_DIR}/gc.\${INSTANCE_NAME}.log:time,pid,tid,level,tags:filecount=30,filesize=8K"
JAVA_OPTS="\${JAVA_OPTS} -XX:+HeapDumpOnOutOfMemoryError"
JAVA_OPTS="\${JAVA_OPTS} -XX:HeapDumpPath=\${JBOSS_LOG_DIR}/dump"
export JAVA_OPTS
EOF
